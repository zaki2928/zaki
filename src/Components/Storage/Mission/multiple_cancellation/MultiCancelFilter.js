...
import MultiCancelList from './MultiCancelList';
import { MultiCancelData, MultiCancelHandler,MultiCancelCriteriaHandler } from '../../../../store/Store';
import axios  from "axios";
import {properties, Properties} from "../../../../Properties/Properties"

const getListOfMissionKLSView = properties.Port + properties.getListOfMissionKLSView

class MultiCancelFilter extends Component {
    constructor(props) {
        super(props);
      this.state = {
          locationList: false,
        ...
        axios
          ...
          .then((response) => {
            if (response.status === 200 && response.data.length !== 0) {
              ...
              MultiCancelCriteriaHandler(criteria)
            } else {
              this.setState({
                locationList: true,
                data: [],
    ...
    render() {
        return (
            ...
            >
              {MultiCancelData.length !== 0 || this.state.locationList === true ? (
                <MultiCancelList backHandler={this.backHandler} data={this.state.data} />
              ) : (
                <div>
                  <span>
                    {" "}
...
export default MultiCancelFilter;